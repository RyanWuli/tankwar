...
import com.tank.mca.nettystudy11_tank.net.TankStopMsg;
...
//        System.out.println(tank.isLived());
        ...
        goodTanks.values().forEach(t -> {
            t.setSPEED(Main.goodTankSpeed);
            t.paint(g);
        });
...
//            System.out.println("released:" + e.getKeyCode() + "-" + e.getKeyChar());
        ...
        private void setMainTankDir() {
            if (!bL && !bU && !bR && !bD) { // 没有按方向键-停止
                ...
                if (!tank.moving) { // tank 停止的时候，开始移动再发消息
                    // 发出 tank 移动的消息,发送自己主 tank 移动信息到服务器
