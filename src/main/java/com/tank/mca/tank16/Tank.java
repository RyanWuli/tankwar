...
import com.tank.mca.tank16.observer.TankFireEvent;
import com.tank.mca.tank16.observer.TankFireHandler;
import com.tank.mca.tank16.observer.TankFireHandlerLog;
import com.tank.mca.tank16.observer.TankFireObserver;
...
//    public static final GameModel gm = GameModel.getInstance();
    ...
    private List<TankFireObserver> observers = new ArrayList<>();

    {
        observers.add(new TankFireHandler());
        observers.add(new TankFireHandlerLog());
    }

    void handlerFireKey() {
        TankFireEvent tfe = new TankFireEvent(this); // 观察的事件
        for (TankFireObserver t : observers) {
            t.actionOnFire(tfe);
        }
    }
}
