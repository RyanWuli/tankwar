...
import com.tank.mca.tank8.abstractfactory.*;
...
//    Tank tank = new Tank(rtx, rty, Dir.LEFT, Group.GOOD, false, this);
    ...
    public List<BaseBullet> bullets = new ArrayList<>(); // 可多颗子弹同时出现，子弹容器
    public List<BaseTank> tanks = new ArrayList<>(); // 坦克的 容器
    public List<BaseExplode> explodes = new ArrayList<>();

//    public GameFactory gameFactory = DefaultFactory.getInstance(); // 单例模式 (工厂模式切换坦克、爆炸、子弹外观)
    public GameFactory gameFactory = RectFactory.getInstance(); // 单例模式 (工厂模式切换坦克、爆炸、子弹外观)
    BaseTank tank = gameFactory.createTank(rtx, rty, Dir.LEFT, Group.GOOD, false, this);
...
//        System.out.println(tank.isLived());
        if (tank.isLived()) {
            ...
            tank.setRectangleX(-100);
            tank.setRectangleY(-100);
        }
