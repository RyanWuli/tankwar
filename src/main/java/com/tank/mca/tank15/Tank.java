...
import com.tank.mca.tank15.decorator.RectDecorator;
import com.tank.mca.tank15.decorator.TailDecorator;
...
//    public static final GameModel gm = GameModel.getInstance();
    ...
    public Tank(int x, int y, Dir dir, Group group, boolean moving) {
        ...
        this.group = group;
        this.moving = moving;
        this.w = ResourceMgr.goodTankU.getWidth();
        this.h = ResourceMgr.goodTankU.getHeight();

        rectangle.x = x;
        rectangle.y = y;
        rectangle.width = ResourceMgr.goodTankU.getWidth();
        rectangle.height = ResourceMgr.goodTankU.getHeight();
    ...
    private void boundCheck() {
        if (x < 2) x = 2;
        if (y < 28) y = 28;
        if (x > TankFrame.GAME_WIDTH - ResourceMgr.goodTankU.getWidth() - 2) x = TankFrame.GAME_WIDTH - ResourceMgr.goodTankU.getWidth() - 2;
        if (y > TankFrame.GAME_HEIGHT - ResourceMgr.goodTankU.getHeight() - 2) y = TankFrame.GAME_HEIGHT - ResourceMgr.goodTankU.getHeight() - 2;
    }
    ...
    // 开火
    public void fire() {
//        GameModel.getInstance().add(new Bullet(this.x + ResourceMgr.goodTankU.getWidth() / 2 - ResourceMgr.bulletU.getWidth() / 2, this.y + ResourceMgr.goodTankU.getHeight() / 2 - ResourceMgr.bulletU.getWidth() / 2, this.dir, this.group));
        // 把装饰后的 object 加进物体里面然后通过装饰器把 物体和装饰一起画出来
        GameModel.getInstance().add(new RectDecorator(new TailDecorator(
