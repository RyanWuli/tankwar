...
import com.tank.mca.tank15.cor.CollisionChain;
...
public class GameModel {
    ...
    public static int tankMaxX = gameWidth - ResourceMgr.goodTankU.getWidth() - 2;
    ...
    int availableX = gameWidth - 40 - ResourceMgr.goodTankU.getWidth(); // 可用 x
...
//    Collision c2 = new TankTankCollision();
    ...
    private void init() {
        tank = new Tank(rtx, rty, Dir.LEFT, Group.GOOD, false);
        int rX = Main.r.nextInt(tankMaxX) % (tankMaxX - tankMinX + 1) + tankMinX;
        int increment = 0;
        // 初始化敌方坦克
        for (int i = 0; i < initBadTankCount; i++) {
            int x = rX + increment;
            increment += Main.r.nextInt(intervalMaxX) % (intervalMaxX - ResourceMgr.goodTankU.getWidth() + 1) + ResourceMgr.goodTankU.getWidth();
...
//            tf.tanks.add(new Tank(i * 150, 150, Dir.DOWN, Group.BAD, true, tf));
            new Tank(x, rY, Dir.DOWN, Group.BAD, true);
        }
